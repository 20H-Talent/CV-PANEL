function Companies() {
    ...
    this.renderTable = function() {
        ...
        for (var i = 0; i < this.companies.length; i++) {
            console.log("algo", i);
            $("#tableBody").append(`<tr scope="row"  data-id=${this.companies[i].id}>    <td class="company-logo user-avatar  pl-5"><img class="img text-center align-middle"src=${this.companies[i].logo} style="width:50px;"/></td>
                 <td class="fullname"><p>${this.companies[i].name}</p></td>
                      ...
                      <button  type="button" onclick=" ${this.companies[i].showInfo()}"  data-id=${this.companies[i].id}   class="btn btn-danger" data-toggle="modal" data-target="#ordine"><i class="far fa-eye"></i></button>
