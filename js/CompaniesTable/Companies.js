...
function Companies() {
    ...
    this.renderCompaniesTable = function(filtredCompanies) {
        ...
        for (var i = 0; i < filtredCompanies.length; i++) {
          ...
          <td class="company-country">${filtredCompanies[i].address.country}</td>
          ...
          <td class="options text-center">
             <button  type="button" onclick="companies.showPreviewInfo('${filtredCompanies[i].id}');" data-toggle="modal" data-id=${
        filtredCompanies[i].id
    ...
    this.renderCompanyCards = function() {
        ...
        for (var i = 0; i < this.companies.length; i++) {
          ...
          <div class="card-footer  header-card   col-sm-12 mt-2 border  text-right rounded">
             ...
             <button type="button" title="Delete company" onclick="companies.modalDelete('${this.companies[i].id}')"  class="btn  remove-company  btn-sm  btn-outline-danger "  data-toggle="modal" data-id=${
             this.companies[i].id
            } data-target="#confirm-delete-card"><i class="fas fa-trash-alt"></i></button>
          </div>
       </div>
