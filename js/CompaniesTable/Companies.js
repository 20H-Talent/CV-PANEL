    ...
    this.renderCompaniesTable = function(filtredCompanies) {
        ...
        for (var i = 0; i < filtredCompanies.length; i++) {
          ...
          <td class="options text-center">
             ...
             <button type="button" rel="tooltip" title="Edit company" id="editCompanyForm"    class="btn btn-sm btn-outline-primary  edit-company"  onclick="editCompany('${filtredCompanies[i].id}');" data-original-title=""  )><i class="fas fa-user-edit"></i>
             </button>
             <button type="button" title="Delete company"  class="btn  btn-sm  btn-outline-danger deleteNow " data-id="${filtredCompanies[i].id}"  onclick="modalDelete('${filtredCompanies[i].id}')"><i class="fas fa-trash-alt"></i></button>
          </td>
       </tr>`;
        }
        $("#tableBody").html("");
    ...
    this.renderCompanyCards = function() {
        ...
        for (var i = 0; i < this.companies.length; i++) {
          ...
          <div class="card-footer  header-card   col-sm-12 mt-2 border  text-right rounded">
             ...
             <button type="button" title="Delete company"  class="btn  remove-company  btn-sm  btn-outline-danger "  data-toggle="modal"  data-id="${this.companies[i].id}"  onclick="modalDelete('${this.companies[i].id}')"><i class="fas fa-trash-alt"></i></button></td>
          </div>
       </div>
       </div>
