    ...
    $.each(data, function(d) {
      $('#skill').append(
        `<div class="custom-control custom-checkbox custom-control-inline">
          <input name="${data[d].name}" type="checkbox" class="custom-control-input" id="${data[d].label}" value=${data[d].defaultValue}>
  ...
  function searchForm(data){
    $.each(data, function(d) {
      let divObjectSkill = `<div class="custom-control custom-checkbox mr-3">
        <input type="checkbox" id="${data[d].label}1" class="custom-control-input" value=${data[d].defaultValue} name="${data[d].name}">
        <label class="custom-control-label" for="${data[d].label}1">${data[d].label}</label>
      </div>`;
